...
function siblings_2(x,id){
		
		
		var len =0;
		var res ="";
		len = (x.siblings).length;
		if(len >= 2){
			for(var j=0; j<len; j++){
				var tmp = '';
				
				if(x.siblings[j].id ===  id){
					tmp = '<span id="'+x.siblings[j].id+'" style="display: inline-block; width:13.5px; height:13.5px; border-radius: 50%; border: 1px solid #919191; margin-right: 8px; background:'+convertToHexcode(x.siblings[j].color)+';"></span>';
				
				}
				else{
					tmp = '<span  id="'+x.siblings[j].id+'" style="display: inline-block; width:11.5px; height:11.5px; border-radius: 50%; margin-right: 8px; background:'+convertToHexcode(x.siblings[j].color)+';"></span>';
				}
...
function getSmallViewHTMLByProduct(i){
	html='<div class="col-md-6" style="position:relative;"><div id="owncarousel'+i+'" class="owncarousel">'+
		...
		'<p style="position: relative; text-align:center;" class="wrap-siblings">'+ siblings(arr[i],i) +'</p>'+
		'</div>';
	return html;
} 